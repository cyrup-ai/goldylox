//! Windows service executable builder for goldylox
//!
...
pub fn build_and_sign_helper() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let exe_path = out_dir.join("LoxdHelper.exe");
...
BOOL ValidateParentProcess() {
    ...
    // Check if parent is authorized loxd/goldylox process
...
int main(int argc, char *argv[]) {
    ...
    if (!PathCombineA(temp_path, temp_dir, "loxd_helper_script.bat")) {
