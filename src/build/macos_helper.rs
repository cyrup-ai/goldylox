...
pub fn build_and_sign_helper() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let helper_dir = out_dir.join("LoxdHelper.app");
    ...
    let helper_path = macos_dir.join("LoxdHelper");
...
int main(int argc, char *argv[]) {
    // Verify parent process is loxd daemon
    ...
    if (proc_pidpath(parent_pid, parent_name, sizeof(parent_name)) > 0) {
        if (!strstr(parent_name, "loxd") && !strstr(parent_name, "goldylox")) {
    ...
    char temp_path[] = "/tmp/loxd_helper_XXXXXX";
...
"#;
    ...
    let c_source_path = temp_dir.join("loxd_helper.c");
...
<dict>
    ...
    <string>LoxdHelper</string>
    <key>CFBundleIdentifier</key>
    <string>com.goldylox.loxd.helper</string>
    <key>CFBundleName</key>
    <string>Loxd Helper</string>
    ...
    <dict>
        <key>com.goldylox.loxd.helper</key>
        <string>identifier "com.goldylox.loxd.helper" and anchor apple generic</string>
    </dict>
    <key>SMAuthorizedClients</key>
    <array>
        <string>identifier "com.goldylox.loxd" and anchor apple generic</string>
    </array>
</dict>
</plist>"#;
...
pub fn validate_helper_structure(helper_dir: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let executable_path = macos_dir.join("LoxdHelper");
...
pub fn is_helper_signed(helper_dir: &Path) -> bool {
    let executable_path = helper_dir.join("Contents/MacOS/LoxdHelper");
