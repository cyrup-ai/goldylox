...
pub fn create_helper_zip(
    ...
    let zip_path = out_dir.join("LoxdHelper.app.zip");
...
/// Create functional ZIP with proper helper app and atomic rollback (replaces placeholder)
#[allow(dead_code)]
pub fn create_functional_zip(zip_path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    // Validate input paths before any operations
    if !zip_path.parent().is_some_and(|p| p.exists()) {
        return Err(format!(
    ...
    let temp_zip_actual = temp_dir.join("LoxdHelper.app.zip");
...
fn verify_zip_integrity(zip_path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let required_files = [
        "LoxdHelper.app/Contents/Info.plist",
        "LoxdHelper.app/Contents/MacOS/LoxdHelper",
    ];
    ...
    for required_file in &required_files {
        ...
        if !found {
            return Err(format!("Required file {} not found in ZIP", required_file).into());
        }
    }
    ...
    for i in 0..archive.len() {
        ...
        if file.name().ends_with("LoxdHelper") && buffer.is_empty() {
