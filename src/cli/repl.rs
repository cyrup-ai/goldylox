...
fn parse_repl_command(input: &str) -> CliResult<Commands> {
    ...
    match command.as_str() {
        "get" => {
            ...
            Ok(Commands::Get { key: parts[1].to_string() })
        }
        "put" => {
            if parts.len() < 3 {
                return Err(CliError::ArgumentError("Usage: put <key> <value>".to_string()));
            }
        ...
        "remove" | "rm" => {
            ...
            Ok(Commands::Remove { key: parts[1].to_string() })
        }
        "clear-cache" => {
            Ok(Commands::Clear)
        }
        "contains" => {
            ...
            Ok(Commands::Contains { key: parts[1].to_string() })
        }
        "hash" => {
            if parts.len() != 2 {
                return Err(CliError::ArgumentError("Usage: hash <key>".to_string()));
            }
            Ok(Commands::Hash { key: parts[1].to_string() })
        }
        "stats" => {
        ...
        "compression-stats" => {
            Ok(Commands::CompressionStats)
        }
        "put-if-absent" => {
            if parts.len() < 3 {
                return Err(CliError::ArgumentError("Usage: put-if-absent <key> <value>".to_string()));
            }
            let key = parts[1].to_string();
            let value = parts[2..].join(" ");
        ...
        "replace" => {
            if parts.len() < 3 {
                return Err(CliError::ArgumentError("Usage: replace <key> <value>".to_string()));
            }
            let key = parts[1].to_string();
            let value = parts[2..].join(" ");
        ...
        "compare-and-swap" | "cas" => {
            if parts.len() < 4 {
                return Err(CliError::ArgumentError("Usage: compare-and-swap <key> <expected> <new_value>".to_string()));
            }
            let key = parts[1].to_string();
            let expected = parts[2].to_string();
            let new_value = parts[3..].join(" ");
            Ok(Commands::CompareAndSwap { key, expected, new_value })
        }
        "get-or-insert" => {
            if parts.len() < 3 {
                return Err(CliError::ArgumentError("Usage: get-or-insert <key> <value>".to_string()));
            }
            let key = parts[1].to_string();
            let value = parts[2..].join(" ");
        ...
        _ => {
            Err(CliError::ArgumentError(format!("Unknown command: {}. Type 'help' for available commands.", command)))
        }
    }
}
