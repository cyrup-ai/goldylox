...
pub fn format_boolean(value: bool, format: &OutputFormat) -> String {
    match format {
        OutputFormat::Human => if value { "true".to_string() } else { "false".to_string() },
        OutputFormat::Json => serde_json::to_string(&value).unwrap_or_else(|_| "false".to_string()),
        OutputFormat::Quiet => if value { "1".to_string() } else { "0".to_string() },
    }
}
...
pub fn format_optional<T: std::fmt::Display>(value: &Option<T>, format: &OutputFormat) -> String {
    match value {
        Some(v) => match format {
            ...
            OutputFormat::Json => serde_json::to_string(&format!("{}", v)).unwrap_or_else(|_| "null".to_string()),
...
pub fn format_hash(hash: u64, format: &OutputFormat) -> String {
    match format {
        ...
        OutputFormat::Json => serde_json::to_string(&format!("0x{:016x}", hash)).unwrap_or_else(|_| "null".to_string()),
...
pub fn format_batch_summary<T: std::fmt::Debug>(
    ...
    match format {
        ...
        OutputFormat::Quiet => {
            format!("{}/{}", summary.successful_results.len(), summary.total_operations)
        }
    }
}
...
pub fn format_stats(stats_json: &str, format: &OutputFormat) -> String {
    match format {
        ...
        OutputFormat::Quiet => {
            ...
            if let Ok(stats) = serde_json::from_str::<serde_json::Value>(stats_json) {
                format!("{:.1}", stats["overall_hit_rate"].as_f64().unwrap_or(0.0) * 100.0)
            } else {
                "0".to_string()
            }
        }
