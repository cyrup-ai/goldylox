...
use crate::cli::{
    ...
    output::{format_batch_summary, format_boolean, format_hash, format_optional, format_stats, format_value, print_output},
};
...
pub enum Commands {
    ...
    CompareAndSwap { key: String, expected: String, new_value: String },
    GetOrInsert { key: String, value: String },
    BatchGet { keys: Vec<String> },
    BatchPut { entries: Vec<String> },
    BatchRemove { keys: Vec<String> },
    Stats { detailed: bool },
    CompressionStats,
    Strategy { action: StrategyAction },
    Tasks { action: TaskAction },
...
pub async fn execute_command(command: Commands, config: CliConfig) -> CliResult<()> {
    ...
    if let Commands::Completions { shell } = command {
        ...
        match shell.to_lowercase().as_str() {
            "bash" => {
                ...
                println!();
                println!("complete -W 'get put remove clear contains hash put-if-absent replace compare-and-swap get-or-insert batch-get batch-put batch-remove stats compression-stats strategy tasks maintenance system config repl completions' lox");
            }
            "zsh" => {
            ...
            "fish" => {
                ...
                println!("complete -c lox -n '__fish_use_subcommand' -a get -d 'Get a value from the cache'");
                println!("complete -c lox -n '__fish_use_subcommand' -a put -d 'Store a key-value pair in the cache'");
                println!("complete -c lox -n '__fish_use_subcommand' -a remove -d 'Remove a key from the cache'");
                println!("complete -c lox -n '__fish_use_subcommand' -a clear -d 'Clear all entries from the cache'");
                println!("complete -c lox -n '__fish_use_subcommand' -a contains -d 'Check if a key exists'");
                println!("complete -c lox -n '__fish_use_subcommand' -a hash -d 'Get hash value for a key'");
                println!("complete -c lox -n '__fish_use_subcommand' -a stats -d 'Get cache statistics'");
                println!("complete -c lox -n '__fish_use_subcommand' -a repl -d 'Start interactive REPL mode'");
            }
            "powershell" | "pwsh" => {
                ...
                println!("    $commands | Where-Object {{{{ $_ -like \"$wordToComplete*\" }}}} | ForEach-Object {{{{");
                println!("        [System.Management.Automation.CompletionResult]::new($_, $_, 'ParameterValue', $_)");
                println!("    }}}}");
                println!("}}}}");
            }
            "elvish" => {
            ...
            _ => {
                return Err(CliError::ArgumentError(format!("Unsupported shell: {}. Supported: bash, zsh, fish, powershell, elvish", shell)));
            }
        }
    ...
    let daemon_endpoint = cli_config.daemon_endpoint
    ...
    match command {
        ...
        Commands::CompareAndSwap { key, expected, new_value } => {
            ...
            let parsed_entries: Result<Vec<(String, String)>, CliError> = entries
                ...
                .map(|entry| {
                    ...
                    if parts.len() == 2 {
                        ...
                        Err(CliError::ArgumentError(format!("Invalid entry format: '{}'. Expected 'key=value'", entry)))
                    }
                })
                .collect();
        ...
        Commands::CompressionStats => {
            ...
            print_output(&compression_stats, &cli_config.output_format, cli_config.quiet);
        }
        ...
        Commands::Strategy { action } => {
            match action {
                ...
                StrategyAction::Force { strategy } => {
                    ...
                    if !cli_config.quiet {
                        print_output(&format!("Strategy forced to: {}", strategy), &cli_config.output_format, false);
                    }
                }
            }
        }
        ...
        Commands::Maintenance { action } => {
            match action {
                ...
                MaintenanceAction::Config => {
                    ...
                    print_output(&maintenance_config, &cli_config.output_format, cli_config.quiet);
                }
                MaintenanceAction::Start => {
                    client.start_background_processor().await?;
                    if !cli_config.quiet {
                        print_output("Background maintenance processor started", &cli_config.output_format, false);
                    }
                }
            }
        }
        ...
        Commands::System { action } => {
            match action {
                ...
                SystemAction::Shutdown => {
                    ...
                    if !cli_config.quiet {
                        print_output("Cache system shutdown gracefully", &cli_config.output_format, false);
                    }
                }
            }
        }
        ...
        Commands::Config { action } => {
            match action {
                ...
                ConfigAction::Update { parameter, value } => {
                    ...
                    if !cli_config.quiet {
                        print_output(&format!("Configuration updated: {} = {}", parameter, value), &cli_config.output_format, false);
                    }
                }
                ConfigAction::Generate { output } => {
                    let default_config = client.get_default_config().await?;
                    std::fs::write(&output, default_config)
                        .map_err(|e| CliError::SystemError(format!("Failed to write config file: {}", e)))?;
                    if !cli_config.quiet {
                        print_output(&format!("Default configuration written to: {}", output.display()), &cli_config.output_format, false);
                    }
                }
            }
        }
        ...
        Commands::Repl => {
            ...
            return Err(CliError::SystemError("REPL mode should be handled before command execution".to_string()));
        }
