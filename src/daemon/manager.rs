...
use crate::{GoldyloxBuilder, Goldylox};
...
impl CacheManager {
    ...
    pub async fn start_api_server(&mut self, cfg: &ServiceConfig) -> Result<()> {
        if let Some(api_config) = &cfg.sse
            && api_config.enabled
        {
            info!("Starting HTTP API server on {}:{}", api_config.host, api_config.port);
            ...
            info!("HTTP API server started on {}:{}", api_config.host, api_config.port);
        }
        Ok(())
    }
...
async fn start_api_server(
    ...
    async fn set_cache_value(
        ...
        match BASE64_STANDARD.decode(&request.value) {
            Ok(decoded_value) => {
                match cache.put(key.clone(), decoded_value.clone()) {
                    Ok(_) => {
                        let response = CacheResponse {
                            success: true,
                            data: Some(CacheValue {
            ...
            match UnifiedCacheStatistics::get_global_instance() {
                Ok(telemetry) => {
                    ...
                    let total_keys = metrics.hot_tier.entry_count.saturating_add(
                        metrics.warm_tier.entry_count.saturating_add(metrics.cold_tier.entry_count)
                    );
                    ...
                    let estimated_uptime_seconds = if ops_per_second > f64::EPSILON {
                        metrics.total_operations.saturating_mul(1000) / (ops_per_second * 1000.0) as u64
                    } else {
                    ...
                    (total_keys, hit_rate, memory_usage_bytes, ops_per_second, estimated_uptime_seconds)
                },
                Err(telemetry_error) => {
                    log::error!("Primary telemetry system failed: {:?}, attempting cache statistics fallback", telemetry_error);
                    ...
                    match cache.stats() {
                        Ok(stats_json) => {
                            match serde_json::from_str::<serde_json::Value>(&stats_json) {
                                Ok(stats_value) => {
                                    log::debug!("Using parsed cache statistics as telemetry fallback");
                                    
                                    // Extract values using verified JSON fields only
                                    let total_keys = {
                                        // Calculate total keys using verified tier hit counts as best proxy
                                        let hot_hits = stats_value["hot_tier_hits"].as_u64().unwrap_or(0);
                                        let warm_hits = stats_value["warm_tier_hits"].as_u64().unwrap_or(0);
                                        let cold_hits = stats_value["cold_tier_hits"].as_u64().unwrap_or(0);
                                    ...
                                    let hit_rate = stats_value["overall_hit_rate"].as_f64().unwrap_or(0.0);
                                    let memory_usage_bytes = stats_value["total_memory_usage"].as_u64().unwrap_or(0);
                                    let ops_per_second = stats_value["ops_per_second"].as_f64().unwrap_or(0.0);
                                    
                                    // Calculate uptime using verified fields with division-by-zero protection
                                    let estimated_uptime_seconds = if ops_per_second > f64::EPSILON {
                                        (stats_value["total_operations"].as_u64().unwrap_or(0) as f64 / ops_per_second) as u64
                                    } else {
                                    ...
                                    (total_keys as usize, hit_rate, memory_usage_bytes, ops_per_second, estimated_uptime_seconds)
                                },
                                Err(parse_error) => {
                                    log::error!("Failed to parse cache statistics JSON - telemetry: {:?}, parsing: {:?}", 
                                               telemetry_error, parse_error);
                                    
                                    // Return error response instead of fake data
                                    return (StatusCode::SERVICE_UNAVAILABLE, Json(CacheResponse::<CacheStats> {
                                        success: false,
                                        data: None,
                                        message: Some(format!("Cache statistics temporarily unavailable: {}", parse_error)),
                                    }));
                                }
                            }
                        },
                        Err(cache_error) => {
                            log::error!("Complete statistics failure - telemetry: {:?}, cache API: {:?}", 
                                       telemetry_error, cache_error);
                            
                            // Return error response instead of fake data  
                            return (StatusCode::SERVICE_UNAVAILABLE, Json(CacheResponse::<CacheStats> {
                                success: false,
                                data: None,
                                message: Some("Cache statistics service completely unavailable".to_string()),
                            }));
                        }
                    }
                }
    ...
    async fn put_if_absent_handler(
        ...
        match BASE64_STANDARD.decode(&request.value) {
            Ok(decoded_value) => {
                match cache.put_if_absent(key.clone(), decoded_value.clone()) {
                    Ok(previous) => {
    ...
    async fn replace_handler(
        ...
        match BASE64_STANDARD.decode(&request.value) {
            Ok(decoded_value) => {
                match cache.replace(key.clone(), decoded_value.clone()) {
                    Ok(previous) => {
                        let response_data = previous.as_ref().map(|prev_val| CacheValue {
                            key: key.clone(),
                            value: BASE64_STANDARD.encode(prev_val),
    ...
    async fn get_or_insert_handler(
        ...
        match BASE64_STANDARD.decode(&request.value) {
            Ok(decoded_value) => {
                match cache.get_or_insert(key.clone(), || decoded_value.clone()) {
                    Ok(result_value) => {
                        let response = CacheResponse {
                            success: true,
                            data: Some(CacheValue {
    ...
    async fn get_detailed_stats(
        ...
        match UnifiedCacheStatistics::get_global_instance() {
            Ok(telemetry) => {
                ...
                let total_keys = metrics.hot_tier.entry_count
                    .saturating_add(metrics.warm_tier.entry_count)
                    .saturating_add(metrics.cold_tier.entry_count);
                
                let total_memory = metrics.hot_tier.memory_usage
            ...
            Err(telemetry_error) => {
                log::error!("Telemetry system failed for detailed stats: {:?}", telemetry_error);
    ...
    async fn batch_get_handler(
        ...
        let response = CacheResponse {
            success: true,
            data: Some(summary),
            message: Some(format!("Batch get completed: {}/{} successful", 
                successful_ops, 
                total_ops)),
        };
        (StatusCode::OK, Json(response))
    ...
    async fn batch_put_handler(
        ...
        let response = CacheResponse {
            success: true,
            data: Some(summary),
            message: Some(format!("Batch put completed: {}/{} successful", 
                successful_ops, 
                total_ops)),
        };
        (StatusCode::OK, Json(response))
    ...
    async fn batch_remove_handler(
        ...
        let response = CacheResponse {
            success: true,
            data: Some(summary),
            message: Some(format!("Batch remove completed: {}/{} successful", 
                successful_ops, 
                total_ops)),
        };
        (StatusCode::OK, Json(response))
    ...
    // Build the router
    let app = Router::new()
        .route("/cache/{key}", get(get_cache_value).put(set_cache_value).delete(delete_cache_value))
        ...
        .route("/cache/batch/remove", axum::routing::post(batch_remove_handler))
