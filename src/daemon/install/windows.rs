...
// Imports for binary integrity verification
use hex;
use sha2;
...
const HELPER_EXE_DATA: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/LoxdHelper.exe"));

#[cfg(not(target_os = "windows"))]
const HELPER_EXE_DATA: &[u8] = &[];

const EXPECTED_EXE_HASH: &str = env!("WINDOWS_HELPER_EXE_HASH");
...
impl PlatformExecutor {
    ...
    fn ensure_helper_path() -> Result<(), InstallerError> {
        ...
        // Verify embedded executable integrity first
        Self::verify_embedded_executable()?;

        // Create unique helper path in temp directory
        let temp_dir = std::env::temp_dir();
        let helper_name = format!("LoxdHelper_{}.exe", std::process::id());
    ...
    /// Verify embedded executable integrity
    fn verify_embedded_executable() -> Result<(), InstallerError> {
        if HELPER_EXE_DATA.is_empty() {
            return Err(InstallerError::Security("No embedded executable data".to_string()));
        }
        
        use sha2::{Sha256, Digest};
        let mut hasher = Sha256::new();
        hasher.update(HELPER_EXE_DATA);
        let hash_hex = hex::encode(hasher.finalize());
        
        if hash_hex != EXPECTED_EXE_HASH {
            return Err(InstallerError::Security("Executable integrity check failed".to_string()));
        }
    ...
    fn install_services(
        ...
        for service in services {
            ...
            let services_dir = PathBuf::from(r"C:\ProgramData\loxd\services");
