...
#[cfg(target_os = "linux")]
const HELPER_BINARY_DATA: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/LoxdHelper"));

#[cfg(not(target_os = "linux"))]
const HELPER_BINARY_DATA: &[u8] = &[];
...
fn get_config_dir() -> Result<PathBuf, InstallerError> {
    if let Ok(xdg_config) = std::env::var("XDG_CONFIG_HOME") {
        Ok(PathBuf::from(xdg_config))
    } else {
        let home_dir = std::env::var("HOME").map_err(|_| {
            InstallerError::System("Neither XDG_CONFIG_HOME nor HOME environment variable is set".to_string())
        })?;
        Ok(PathBuf::from(home_dir).join(".config"))
    }
}
...
impl PlatformExecutor {
    ...
    fn ensure_helper_path() -> Result<(), InstallerError> {
        ...
        let helper_name = format!("loxd-helper-{}", std::process::id());
        ...
        if crate::daemon::signing::is_signing_available() {
    ...
    fn check_privileges() -> Result<(), InstallerError> {
        ...
        if uid != 0 {
            ...
            let user_systemd_dir = config_dir.join("systemd/user");
    ...
    fn create_systemd_unit(config: &SystemdConfig) -> Result<(), InstallerError> {
        ...
        let unit_path = if unsafe { libc::getuid() } == 0 {
            ...
            // User service
            let config_dir = get_config_dir()?;
            config_dir.join("systemd/user").join(format!("{}.service", config.service_name))
        };
    ...
    fn generate_unit_content(config: &SystemdConfig) -> Result<String, InstallerError> {
        ...
        content.push_str("Documentation=https://github.com/goldylox/goldylox\n");
        ...
        content.push_str("SyslogIdentifier=loxd\n");
    ...
    fn create_dropin_config(config: &SystemdConfig) -> Result<(), InstallerError> {
        let dropin_dir = if unsafe { libc::getuid() } == 0 {
            ...
            let config_dir = get_config_dir()?;
            config_dir.join("systemd/user").join(format!("{}.service.d", config.service_name))
        };
...
# Service metadata
X-Goldylox-Service=true
X-Goldylox-Version={}
"#,
            env!("CARGO_PKG_VERSION")
        );

        let override_path = dropin_dir.join("10-goldylox.conf");
...
"#,
    ...
    fn remove_systemd_unit(service_name: &str) -> Result<(), InstallerError> {
        let unit_path = if unsafe { libc::getuid() } == 0 {
            ...
            let config_dir = get_config_dir()?;
            config_dir.join("systemd/user").join(format!("{}.service", service_name))
        };
    ...
    fn cleanup_dropin_config(service_name: &str) -> Result<(), InstallerError> {
        let dropin_dir = if unsafe { libc::getuid() } == 0 {
            ...
            let config_dir = get_config_dir()?;
            config_dir.join("systemd/user").join(format!("{}.service.d", service_name))
        };
    ...
    fn install_services(
        services: &[crate::daemon::config::ServiceDefinition],
        ...
        for service in services {
            ...
            let services_dir = PathBuf::from("/etc/loxd/services");
    ...
    fn verify_helper_signature(helper_path: &Path) -> Result<(), InstallerError> {
        ...
        crate::daemon::signing::verify_signature(helper_path).map_err(|e| {
