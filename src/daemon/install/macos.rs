...
impl PlatformExecutor {
    ...
    fn validate_embedded_zip() -> Result<(), InstallerError> {
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            if hash_hex != EXPECTED_ZIP_HASH {
                return Err(InstallerError::Security("ZIP integrity check failed".to_string()));
            }
        }
    ...
    fn validate_zip_structure(zip_data: &[u8]) -> Result<(), InstallerError> {
        ...
        for i in 0..archive.len() {
            let file = archive.by_index(i)
                .map_err(|e| InstallerError::System(format!("Failed to read ZIP entry {}: {}", i, e)))?;
        ...
        if !has_info_plist {
            return Err(InstallerError::System("ZIP missing required Info.plist".to_string()));
        }
        
        if !has_executable {
            return Err(InstallerError::System("ZIP missing required executable".to_string()));
        }
    ...
    fn extract_zip_data(zip_data: &[u8], target_path: &PathBuf) -> Result<(), InstallerError> {
        ...
        if zip_data.is_empty() {
            return Err(InstallerError::System("ZIP data is empty - build script may have failed".to_string()));
        }
        ...
        std::fs::create_dir_all(target_path.parent().unwrap_or(target_path))
            .map_err(|e| InstallerError::System(format!("Failed to create target directory: {}", e)))?;
        ...
        for i in 0..archive.len() {
            ...
            if !outpath.starts_with(target_path) {
                return Err(InstallerError::Security(
                    format!("Unsafe file path in ZIP: {}", file.name())
                ));
            }
            ...
            if file.name().ends_with('/') {
                ...
                std::fs::create_dir_all(&outpath)
                    .map_err(|e| InstallerError::System(format!("Failed to create directory {}: {}", outpath.display(), e)))?;
            } else {
                // File entry - ensure parent directory exists
                if let Some(parent) = outpath.parent() {
                    std::fs::create_dir_all(parent)
                        .map_err(|e| InstallerError::System(format!("Failed to create parent directory {}: {}", parent.display(), e)))?;
                }
                
                // Extract file with proper permissions
                let mut outfile = std::fs::File::create(&outpath)
                    .map_err(|e| InstallerError::System(format!("Failed to create file {}: {}", outpath.display(), e)))?;
                
                std::io::copy(&mut file, &mut outfile)
                    .map_err(|e| InstallerError::System(format!("Failed to extract file {}: {}", outpath.display(), e)))?;
                ...
                #[cfg(unix)]
                {
                    ...
                    if let Some(mode) = file.unix_mode() {
                        std::fs::set_permissions(&outpath, std::fs::Permissions::from_mode(mode))
                            .map_err(|e| InstallerError::System(format!("Failed to set permissions on {}: {}", outpath.display(), e)))?;
                    }
                }
            }
        }
    ...
    fn verify_code_signature(helper_path: &Path) -> Result<bool, InstallerError> {
        ...
        if !plist_content.contains("com.goldylox.loxd.helper") {
            return Err(InstallerError::System(
                "Bundle identifier validation failed - expected com.goldylox.loxd.helper".to_string(),
            ));
        }
