...
fn load_config_file() -> Result<SigningConfigFile> {
    ...
    let search_paths = vec![
        ...
        // Project root
        PathBuf::from("loxd/signing.toml"),
        // User config directory
        dirs::config_dir()
            .map(|p| p.join("loxd/signing.toml"))
            .unwrap_or_default(),
        // System config
        PathBuf::from("/etc/loxd/signing.toml"),
    ];
...
fn override_from_env(config: &mut SigningConfig) {
    ...
    if let Ok(bin_path) = env::var("LOXD_BINARY_PATH") {
        config.binary_path = PathBuf::from(bin_path);
    }
    if let Ok(out_path) = env::var("LOXD_OUTPUT_PATH") {
