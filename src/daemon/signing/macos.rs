...
/// Trait for safe path-to-string conversions
trait PathExt {
    fn to_str_safe(&self, operation: &str) -> Result<&str>;
    #[allow(dead_code)] // Complete PathExt API for safe string conversions
    fn to_string_safe(&self, operation: &str) -> Result<String>;
    #[allow(dead_code)] // Complete PathExt API for safe string conversions  
...
impl PathExt for Path {
    fn to_str_safe(&self, operation: &str) -> Result<&str> {
        self.to_str().ok_or_else(|| {
            anyhow::anyhow!(
                "Path contains non-UTF8 characters for {}: {}",
                operation,
                self.display()
            )
        })
    ...
    fn to_string_safe(&self, operation: &str) -> Result<String> {
        Ok(self.to_str_safe(operation)?.to_string())
    }
...
pub fn verify(binary_path: &Path) -> Result<bool> {
    let output = Command::new("codesign")
        .args([
            ...
            "--strict",
            "--verbose=2",
            binary_path.to_str_safe("code signature verification")?,
        ])
        .output()
...
fn notarize(config: &SigningConfig) -> Result<()> {
    ...
    let output = Command::new("ditto")
        .args([
            ...
            "-k",
            "--keepParent",
            config.binary_path.to_str_safe("ZIP creation")?,
            zip_path.to_str_safe("ZIP creation")?,
        ])
        .output()
...
fn staple_ticket(binary_path: &Path) -> Result<()> {
    let output = Command::new("xcrun")
        .args(["stapler", "staple", binary_path.to_str_safe("notarization stapling")?])
        .output()
...
pub fn import_certificate(cert_path: &Path, password: Option<&str>) -> Result<String> {
    ...
    let keychain_name = format!("loxd-signing-{}.keychain", std::process::id());
    ...
    let mut import_args = vec![
        "import",
        cert_path.to_str_safe("certificate import")?,
        "-k",
        &keychain_name,
        "-T",
        "/usr/bin/codesign",
    ...
    unsafe { env::set_var("LOXD_TEMP_KEYCHAIN", &keychain_name); }
...
pub fn cleanup_keychain() -> Result<()> {
    if let Ok(keychain_name) = env::var("LOXD_TEMP_KEYCHAIN") {
        let _ = Command::new("security")
            .args(["delete-keychain", &keychain_name])
            .output();

        unsafe { env::remove_var("LOXD_TEMP_KEYCHAIN"); }
    }
    Ok(())
}
