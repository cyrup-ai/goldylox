//! Code signing module for goldylox daemon
//!
...
impl Default for SigningConfig {
    fn default() -> Self {
        let binary_path = env::current_exe().unwrap_or_else(|_| PathBuf::from("loxd"));
...
impl std::fmt::Display for SigningConfig {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "SigningConfig {{ binary_path: {:?}, output_path: {:?}, platform: {:?} }}", 
               self.binary_path, self.output_path, self.platform)
    }
}
...
pub fn sign_binary(config: &SigningConfig) -> Result<()> {
    match &config.platform {
        ...
        _ => {
            // Graceful fallback for unknown platforms - perform no-op signing for development
            log::warn!("Code signing not available on this platform: {:?}", std::env::consts::OS);
            Ok(()) // Allow development on unsupported platforms
        }
    }
}
