...
pub fn verify(binary_path: &Path) -> Result<bool> {
    ...
    if gpg_path.exists() {
        let gpg_path_str = gpg_path.to_str()
...
pub fn sign_appimage(appimage_path: &Path, key_id: Option<&str>) -> Result<()> {
    ...
    let metadata = appimage_path.metadata()
    ...
    if file_size > MAX_APPIMAGE_SIZE {
        bail!("AppImage too large: {} bytes (max {} bytes)", file_size, MAX_APPIMAGE_SIZE);
    }

    // Memory-safe AppImage format validation using buffered reading
    let file = File::open(appimage_path)
        .context("Failed to open AppImage file")?;
    ...
    println!("Successfully created detached AppImage signature: {}", sig_path.display());
    Ok(())
}
