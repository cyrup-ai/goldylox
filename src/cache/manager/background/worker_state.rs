        ...
        while let Ok(status) = self.status_receiver.try_recv() {
            ...
            if let Ok(mut cached) = self.cached_status.write() {
                *cached = Some(status.clone());
                self.last_update.store(status.snapshot_time_ns, std::sync::atomic::Ordering::Relaxed);
            }
        }
...
impl BackgroundWorkerState {
    ...
    pub fn create_status_snapshot(&self) -> WorkerStatusSnapshot {
        ...
        WorkerStatusSnapshot {
            ...
            tasks_processed: self.tasks_processed.load(std::sync::atomic::Ordering::Relaxed),
            total_processing_time: self.total_processing_time.load(std::sync::atomic::Ordering::Relaxed),
            status: self.status.load(),
            last_heartbeat_ns: self.last_heartbeat.load().elapsed().as_nanos() as u64,
            current_task_discriminant: self.current_task_discriminant.load(std::sync::atomic::Ordering::Relaxed),
            error_count: self.error_count.load(std::sync::atomic::Ordering::Relaxed),
            steal_attempts: self.steal_attempts.load(std::sync::atomic::Ordering::Relaxed),
            successful_steals: self.successful_steals.load(std::sync::atomic::Ordering::Relaxed),
            tasks_since_heartbeat: self.tasks_since_heartbeat.load(std::sync::atomic::Ordering::Relaxed),
    ...
    pub fn register_worker_channel(worker_id: u32, status_receiver: Receiver<WorkerStatusSnapshot>) {
    ...
    pub fn get_all_worker_task_counts() -> Vec<(u32, u64)> {
        ...
        registry
            ...
            .filter_map(|entry| {
                ...
                channel.get_latest_status().map(|status| (worker_id, status.tasks_processed))
            })
            .collect()
    }
    ...
    pub fn get_all_worker_health() -> Vec<(u32, bool)> {
        ...
        registry
            ...
            .filter_map(|entry| {
                let worker_id = *entry.key();
                let channel = entry.value();
                channel.get_latest_status().map(|status| (worker_id, status.is_healthy))
            })
            .collect()
    }
