...
use crate::cache::coherence::data_structures::{CoherenceController, ProtocolConfiguration, CacheTier};
...
pub struct CoherenceWorker<K: CacheKey + Default + bincode::Encode + bincode::Decode<()> + serde::Serialize + serde::de::DeserializeOwned, V: CacheValue + Default + bincode::Encode + bincode::Decode<()> + serde::Serialize + serde::de::DeserializeOwned> {
...
impl<K: CacheKey + Default + bincode::Encode + bincode::Decode<()> + serde::Serialize + serde::de::DeserializeOwned + 'static, V: CacheValue + Default + bincode::Encode + bincode::Decode<()> + serde::Serialize + serde::de::DeserializeOwned + 'static> CoherenceWorker<K, V> {
    ...
    pub fn new(
        ...
        Self {
            controller: CoherenceController::new(config),
            request_rx,
            response_tx,
            shutdown_rx,
        }
    ...
    fn process_request(&self, request: CoherenceRequest<K, V>) -> CoherenceResponse<K, V> {
        match request {
            CoherenceRequest::Read { key, requesting_tier, request_id } => {
                match self.controller.handle_read_request(&key, requesting_tier) {
                    Ok(read_response) => CoherenceResponse::ReadSuccess { 
                        request_id, 
                        response: read_response 
                    },
                    Err(error) => CoherenceResponse::Error { 
                        request_id, 
                        error 
                    },
                }
            }
            CoherenceRequest::Write { key, data, requesting_tier, request_id } => {
                match self.controller.handle_write_request(&key, requesting_tier, data) {
                ...
                CoherenceResponse::Statistics { request_id, statistics: stats }
            }
    ...
    fn create_serialization_envelope_internal(&self, key: K, value: V, target_tier: CacheTier) -> Result<SerializationEnvelope<K, V>, CoherenceError> {
        use crate::cache::traits::cache_entry::{SerializationEnvelope, TierSerializationContext, CacheEntry};
            ...
            self.controller.cache_line_states.get(&coherence_key)
        {
            ...
            let checksum = cache_line.metadata.invalidation_seq.load(std::sync::atomic::Ordering::Acquire) as u64;

            let compression_used = match initial_tier {
                TierLocation::Hot => crate::cache::traits::supporting_types::CompressionAlgorithm::None,
                TierLocation::Warm => crate::cache::traits::supporting_types::CompressionAlgorithm::Lz4,
                TierLocation::Cold => crate::cache::traits::supporting_types::CompressionAlgorithm::Zstd,
            };
