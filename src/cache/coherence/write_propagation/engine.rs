...
use super::types::{
    PropagationConfig, PropagationPolicy, PropagationStatistics,
...
impl<K: CacheKey + Default + bincode::Encode + bincode::Decode<()> + serde::Serialize + serde::de::DeserializeOwned, V: CacheValue + Default + bincode::Encode + bincode::Decode<()> + serde::Serialize + serde::de::DeserializeOwned> WritePropagationSystem<K, V> {
    ...
    pub fn new(writeback_delay_ns: u64) -> Self {
        ...
        let config = if writeback_delay_ns < 50_000_000 { // < 50ms
            PropagationConfig::low_latency()
        } else if writeback_delay_ns > 200_000_000 { // > 200ms
        ...
        let initial_policy = if writeback_delay_ns < 100_000_000 { // < 100ms
    ...
    pub fn submit_writeback(
        ...
        let current_policy = match self.propagation_policy.load(std::sync::atomic::Ordering::Relaxed) {
    ...
    where 
        ...
        V: Default + serde::Serialize + serde::de::DeserializeOwned + bincode::Encode + bincode::Decode<()> + 'static,
    {
        ...
        match write_result {
            Ok(_) => {
                ...
                log::debug!("Write-through completed successfully to {:?} tier", request.target_tier);
            }
            Err(e) => {
                // Write failed - record error
                self.propagation_stats.record_failed_write();
                log::error!("Write-through failed to {:?} tier: {}", request.target_tier, e);
            }
        }
    }
    ...
    pub fn process_worker_completions(&self) {
        ...
        while let Ok(completion) = self.worker_channels.completion_rx.try_recv() {
            ...
            self.propagation_stats.update_write_latency(completion.processing_time_ns);
        }
    ...
    fn adapt_propagation_policy(&self) {
        ...
        let should_use_adaptive = queue_depth > (self.config.max_queue_size / 2) || failure_rate > 5.0;
        
        if should_use_adaptive {
            // Update to adaptive policy for better performance under load
            self.propagation_policy.store(PropagationPolicy::Adaptive as u8, std::sync::atomic::Ordering::Relaxed);
        }
    }
