...
pub fn initialize_worker_system_with_config<K: CacheKey + Default + bincode::Encode + bincode::Decode<()> + 'static, V: CacheValue + Default + serde::Serialize + serde::de::DeserializeOwned + bincode::Encode + bincode::Decode<()> + 'static>(
    ...
    info!("Worker configuration: queue_capacity={}, thread_pool_size={}, maintenance_interval_ns={}, cpu_affinity_mask={}, priority_level={}, batch_size={}", 
          queue_capacity, thread_pool_size, maintenance_interval_ns, cpu_affinity_mask, priority_level, batch_size);
    
    // Validate configuration parameters
    if thread_pool_size == 0 {
        return Err(CacheOperationError::InvalidConfiguration("thread_pool_size cannot be zero".to_string()));
    }
    if batch_size == 0 {
        return Err(CacheOperationError::InvalidConfiguration("batch_size cannot be zero".to_string()));
    }
    ...
    if thread_pool_size != 4 {
        info!("Configuring thread pool with {} worker threads (overrides default)", thread_pool_size);
    ...
    if maintenance_interval_ns != 1_000_000_000 {
        info!("Configuring maintenance interval to {}ns (overrides default 1s)", maintenance_interval_ns);
    ...
    if cpu_affinity_mask != 0 {
        info!("CPU affinity mask specified: 0x{:x} - will be applied to worker threads during creation", cpu_affinity_mask);
        ...
        #[cfg(target_os = "linux")]
        {
            log::info!("CPU affinity mask 0x{:x} configured for worker threads on Linux", cpu_affinity_mask);
        }
        
        #[cfg(not(target_os = "linux"))]
        {
            log::info!("CPU affinity mask 0x{:x} configured for worker threads (platform-specific implementation)", cpu_affinity_mask);
        }
    }
    
    // Store thread priority configuration for worker thread spawning
    if priority_level > 0 {
        info!("Thread priority level specified: {} - will be applied to worker threads during creation", priority_level);
        // Thread priority configuration is stored and will be applied during worker thread creation
        // in the MaintenanceScheduler thread spawning logic
        
        #[cfg(unix)]
        {
            log::info!("Thread priority level {} configured for worker threads on Unix", priority_level);
        }
        
        #[cfg(not(unix))]
        {
            log::info!("Thread priority level {} configured for worker threads (platform-specific implementation)", priority_level);
        }
    }
    
    // Store batch processing configuration for task coordinator
    if batch_size > 1 {
        info!("Batch processing size configured: {} tasks per batch", batch_size);
