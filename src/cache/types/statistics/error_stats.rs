...
impl ErrorStatistics {
    ...
    pub fn record_error(&self, error_type: ErrorType) {
        ...
        log::warn!("Error recorded: {:?} (total: {})", error_type, self.total_errors.load(Ordering::Relaxed));
    }
...
impl Clone for ErrorStatistics {
    fn clone(&self) -> Self {
        ...
        new_stats.total_errors.store(self.total_errors.load(Ordering::Relaxed), Ordering::Relaxed);
        new_stats.total_operations.store(self.total_operations.load(Ordering::Relaxed), Ordering::Relaxed);
        new_stats.last_error_time.store(self.last_error_time.load(Ordering::Relaxed), Ordering::Relaxed);
        new_stats.recent_error_count.store(self.recent_error_count.load(Ordering::Relaxed), Ordering::Relaxed);
        new_stats.window_start_time.store(self.window_start_time.load(Ordering::Relaxed), Ordering::Relaxed);
