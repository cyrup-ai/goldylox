...
impl SimdHasher {
    ...
    fn hash_batch_keys_avx2<K: CacheKey>(&mut self, keys: &[K]) -> Vec<u64> {
        ...
        for chunk in keys.chunks(4) {
            ...
            let key_bytes: Vec<Vec<u8>> = chunk.iter()
                .map(|key| self.get_key_bytes(key))
                .collect();
            ...
            unsafe {
                for chunk_offset in (0..aligned_len).step_by(32) {
                    ...
                    for (key_idx, bytes) in aligned_bytes.iter().enumerate().take(chunk.len()) {
                        ...
                        data_vectors[key_idx] = _mm256_loadu_si256(chunk_data.as_ptr() as *const __m256i);
                    }
                    
                    // Convert all 4 hash states to AVX2 format for parallel processing
                    let state_vectors = [
                        _mm256_broadcast_si128(&_mm_loadu_si128(parallel_states[0].as_ptr() as *const __m128i)),
                        _mm256_broadcast_si128(&_mm_loadu_si128(parallel_states[1].as_ptr() as *const __m128i)),
                        _mm256_broadcast_si128(&_mm_loadu_si128(parallel_states[2].as_ptr() as *const __m128i)),
                        _mm256_broadcast_si128(&_mm_loadu_si128(parallel_states[3].as_ptr() as *const __m128i)),
                    ];
                    ...
                    for (i, &result) in mixed_results.iter().enumerate().take(chunk.len()) {
                        ...
                        _mm_storeu_si128(parallel_states[i].as_mut_ptr() as *mut __m128i, final_state);
                    }
                }
                
                // Phase 5: Extract final hash values from all parallel states
                for i in 0..chunk.len() {
                    let final_hash = (parallel_states[i][0] as u64) << 32 | (parallel_states[i][1] as u64);
            ...
            unsafe {
                for chunk_offset in (0..aligned_len).step_by(16) {
                    ...
                    for (key_idx, bytes) in aligned_bytes.iter().enumerate().take(chunk.len()) {
                        ...
                        data_vectors[key_idx] = _mm_loadu_si128(chunk_data.as_ptr() as *const __m128i);
                    }
                    ...
                    unsafe {
                        ...
                        for chunk_offset in (0..aligned_len).step_by(16) {
                            ...
                            for (i, bytes) in aligned_bytes.iter().enumerate().take(chunk.len()) {
                                ...
                                data_vectors[i] = _mm_loadu_si128(chunk_data.as_ptr() as *const __m128i);
                            }
                            
                            // Convert hash states to SSE2 format for simultaneous processing
                            let state_vectors = [
                                _mm_loadu_si128(hash_states[0].as_ptr() as *const __m128i),
                                if chunk.len() > 1 { _mm_loadu_si128(hash_states[1].as_ptr() as *const __m128i) } else { _mm_setzero_si128() },
                                if chunk.len() > 2 { _mm_loadu_si128(hash_states[2].as_ptr() as *const __m128i) } else { _mm_setzero_si128() },
                                if chunk.len() > 3 { _mm_loadu_si128(hash_states[3].as_ptr() as *const __m128i) } else { _mm_setzero_si128() },
                            ];
                            
                            // SIMULTANEOUS XOR operations for all byte arrays
                            let xor_results = [
                                _mm_xor_si128(state_vectors[0], data_vectors[0]),
                                if chunk.len() > 1 { _mm_xor_si128(state_vectors[1], data_vectors[1]) } else { _mm_setzero_si128() },
                                if chunk.len() > 2 { _mm_xor_si128(state_vectors[2], data_vectors[2]) } else { _mm_setzero_si128() },
                                if chunk.len() > 3 { _mm_xor_si128(state_vectors[3], data_vectors[3]) } else { _mm_setzero_si128() },
                            ];
                            
                            // SIMULTANEOUS mixing operations for all byte arrays
                            let mixed_results = [
                                self.simd_mix(xor_results[0]),
                                if chunk.len() > 1 { self.simd_mix(xor_results[1]) } else { _mm_setzero_si128() },
                                if chunk.len() > 2 { self.simd_mix(xor_results[2]) } else { _mm_setzero_si128() },
                                if chunk.len() > 3 { self.simd_mix(xor_results[3]) } else { _mm_setzero_si128() },
                            ];
                            
                            // Update all hash states simultaneously
                            for (i, &result) in mixed_results.iter().enumerate().take(chunk.len()) {
                                _mm_storeu_si128(hash_states[i].as_mut_ptr() as *mut __m128i, result);
                            }
                        }
                        
                        // Extract final hash values from all hash states
                        for i in 0..chunk.len() {
                            let final_hash = (hash_states[i][0] as u64) << 32 | (hash_states[i][1] as u64);
                            results.push(final_hash);
                        }
