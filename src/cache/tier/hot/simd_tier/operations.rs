...
impl<K: CacheKey + Default, V: CacheValue> SimdHotTier<K, V> {
    ...
    pub fn get(&mut self, key: &K) -> Option<Arc<V>> {
        ...
        self.eviction_engine
            .record_access(0, false, timer.elapsed_ns());
        None
    }
    ...
    pub fn put(&mut self, key: K, value: Arc<V>) -> Result<(), CacheOperationError> {
        ...
        if self.memory_pool.total_memory_usage() + value.estimated_size()
            > self.config.memory_limit as usize
        {
        ...
        if let Some(candidate) = self.eviction_engine.find_eviction_candidate(
            ...
            // Evict the candidate
            self.memory_pool.clear_slot(candidate.slot_index);
            self.eviction_engine
                .record_eviction(candidate.slot_index, true, timer.elapsed_ns());
            ...
            self.lru_tracker.record_access(candidate.slot_index);
