...
pub fn put_if_absent_atomic<K: CacheKey + Default + bincode::Encode + bincode::Decode<()> + 'static, V: CacheValue + Default + serde::Serialize + serde::de::DeserializeOwned + bincode::Encode + bincode::Decode<()> + 'static>(
    key: K, 
    value: V
    ...
    let op = Box::new(move |tier: &mut dyn std::any::Any| -> Vec<u8> {
        if let Some(cold_tier) = tier.downcast_mut::<crate::cache::tier::cold::data_structures::PersistentColdTier<K, V>>() {
            ...
            if let Some(existing) = cold_tier.get(&key_clone) {
                ...
                match cold_tier.put(key_clone, value_clone) {
                    Ok(_) => bincode::encode_to_vec(&None::<V>, bincode::config::standard()).unwrap_or_default(),
                    Err(_) => bincode::encode_to_vec(&None::<V>, bincode::config::standard()).unwrap_or_default()
                }
            }
        } else {
    ...
    coordinator.sender().send(request)
        .map_err(|_| CacheOperationError::TierOperationFailed)?;
        
    let result_bytes = response_rx.recv_timeout(Duration::from_millis(500))
        .map_err(|_| CacheOperationError::TierOperationFailed)??;
        
    bincode::decode_from_slice(&result_bytes, bincode::config::standard())
        .map(|(result, _)| result)
        .map_err(|_| CacheOperationError::SerializationError("Failed to decode put_if_absent result".to_string()))
}

/// Atomically replace existing value with new value in cold tier
pub fn replace_atomic<K: CacheKey + Default + bincode::Encode + bincode::Decode<()> + 'static, V: CacheValue + Default + serde::Serialize + serde::de::DeserializeOwned + bincode::Encode + bincode::Decode<()> + 'static>(
    key: K, 
    value: V
) -> Result<Option<V>, CacheOperationError> {
    use crate::cache::tier::cold::{ColdTierCoordinator, ColdTierMessage};
    ...
    let op = Box::new(move |tier: &mut dyn std::any::Any| -> Vec<u8> {
        if let Some(cold_tier) = tier.downcast_mut::<crate::cache::tier::cold::data_structures::PersistentColdTier<K, V>>() {
            ...
            if old_value.is_some() {
                match cold_tier.put(key_clone, value_clone) {
                    Ok(_) => bincode::encode_to_vec(&old_value, bincode::config::standard()).unwrap_or_default(),
                    Err(_) => bincode::encode_to_vec(&None::<V>, bincode::config::standard()).unwrap_or_default()
                }
            } else {
                bincode::encode_to_vec(&None::<V>, bincode::config::standard()).unwrap_or_default()
    ...
    bincode::decode_from_slice(&result_bytes, bincode::config::standard())
        .map(|(result, _)| result)
        .map_err(|_| CacheOperationError::SerializationError("Failed to decode replace result".to_string()))
}

/// Atomically compare and swap value if current equals expected in cold tier
pub fn compare_and_swap_atomic<K: CacheKey + Default + bincode::Encode + bincode::Decode<()> + 'static, V: CacheValue + Default + serde::Serialize + serde::de::DeserializeOwned + bincode::Encode + bincode::Decode<()> + PartialEq + 'static>(
    key: K, 
    expected: V, 
    new_value: V
    ...
    bincode::decode_from_slice(&result_bytes, bincode::config::standard())
        .map(|(result, _)| result)
        .map_err(|_| CacheOperationError::SerializationError("Failed to decode compare_and_swap result".to_string()))
}