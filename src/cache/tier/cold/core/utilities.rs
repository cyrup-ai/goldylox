...
impl<K: CacheKey + Default + bincode::Encode + bincode::Decode<()>, V: CacheValue + Default + serde::Serialize + serde::de::DeserializeOwned + bincode::Encode + bincode::Decode<()> + 'static> PersistentColdTier<K, V> {
    ...
    pub(super) fn mark_space_free(&self, _offset: u64, _size: u32) {
        ...
        if self.should_compact(fragmentation_threshold) {
            ...
            if let Some(sender) = self.get_maintenance_sender() {
                ...
                let task = crate::cache::manager::background::types::MaintenanceTask::new(canonical_task);
    ...
    fn should_compact(&self, fragmentation_threshold: f64) -> bool {
        ...
        let deleted_entry_bytes = if self.compaction_system.is_compacting() { 
            ...
            match MemoryEfficiencyAnalyzer::new(&cache_config) {
                Ok(analyzer) => {
                    match analyzer.analyze_efficiency() {
                        Ok(analysis) => {
                            ...
                            fragmented_bytes.saturating_mul((analysis.fragmentation_impact * 1000.0) as u64) / 1000
                        },
                        Err(_) => {
                ...
                Err(analyzer_error) => {
                    log::debug!("MemoryEfficiencyAnalyzer failed: {:?}, using defragmentation measurement", analyzer_error);
                    ...
                    match trigger_defragmentation() {
                        Ok(actual_bytes_reclaimed) => {
                            log::debug!("Using measured defragmentation: {} bytes", actual_bytes_reclaimed);
                            actual_bytes_reclaimed as u64
                        },
                        Err(defrag_error) => {
                            log::warn!("Both MemoryEfficiencyAnalyzer and defragmentation failed - analyzer: {:?}, defrag: {:?}", 
                                     analyzer_error, defrag_error);
    ...
    fn get_maintenance_sender(&self) -> Option<&crossbeam_channel::Sender<crate::cache::manager::background::types::MaintenanceTask>> {
    ...
    pub fn schedule_compaction_if_needed(&self, fragmentation_threshold: f64) -> Result<bool, crate::cache::traits::types_and_enums::CacheOperationError> {
        ...
        let compaction_task = crate::cache::manager::background::types::MaintenanceTask::new(canonical_task);
        
        // Submit the task via channel
        match sender.send(compaction_task) {
            Ok(()) => {
                log::info!("Submitted compaction task for cold tier with threshold {:.2}", fragmentation_threshold);
                Ok(true)
            }
            Err(_) => {
                log::error!("Failed to submit compaction task to maintenance scheduler");
                Err(crate::cache::traits::types_and_enums::CacheOperationError::internal_error(
                    "Failed to submit compaction task - scheduler channel may be closed".to_string()
                ))
            }
        }
    }
