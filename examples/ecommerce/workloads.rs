...
pub fn generate_black_friday_rush(workload: &WorkloadState) -> Result<(), Box<dyn std::error::Error>> {
                ...
                if let Some(product) = nodes_clone[node_idx].product_cache.get(&cache_key) {
                    ...
                    if updated_product.inventory_count > 0 {
                        ...
                        let _ = nodes_clone[node_idx].product_cache.compare_and_swap(
                            cache_key, original_product, updated_product
                        );
                    }
                }
...
pub fn generate_regular_browsing(workload: &WorkloadState) -> Result<(), Box<dyn std::error::Error>> {
        ...
        let handle = thread::spawn(move || {
            ...
            for session in 0..200 {
                ...
                for _ in 0..rng.random_range(5..20) {
                    ...
                    if nodes_clone[node_idx].product_cache.get(&cache_key).is_some() {
                        ...
                        let _ = nodes_clone[node_idx].analytics_cache.put(event_id, analytics_event);
                    }
                }
