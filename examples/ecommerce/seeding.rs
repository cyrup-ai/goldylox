...
fn generate_product_metadata(rng: &mut ThreadRng) -> BTreeMap<String, String> {
    ...
    metadata.insert("weight".to_string(), format!("{:.1}", rng.random_range(0.1..50.0)));
    metadata.insert("dimensions".to_string(), format!("{}x{}x{}", 
        rng.random_range(1..100), rng.random_range(1..100), rng.random_range(1..100)));
...
fn generate_browsing_history(rng: &mut ThreadRng) -> Vec<u64> {
    ...
    (0..history_size).map(|_| rng.random_range(1..10_001)).collect()
}
